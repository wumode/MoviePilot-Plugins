...
from .models.api import RuleData, Connectivity, Subscription, RuleProviderData, SubscriptionInfo, HostRequest
...
class ClashRuleProviderApi:
    ...
    def __init__(self, plugin: _ClashRuleProviderBase, services: ClashRuleProviderService):
    ...
    def get_ruleset(self, name: str, apikey: str) -> PlainTextResponse:
        _apikey = self.plugin.config.apikey or settings.API_TOKEN
    ...
    def update_subscription_info(self, sub_info: SubscriptionInfo):
        self.update_subscription_info(sub_info)
        return schemas.Response(success=True)
    ...
    async def clash_websocket(self, request: Request, endpoint: str, secret: str):
        if secret != self.plugin.config.dashboard_secret:
        ...
        queue = asyncio.Queue()
        ws_base = self.plugin.config.dashboard_url.replace(
            'http://', 'ws://').replace('https://', 'wss://')
        url = f"{ws_base}/{endpoint}?token={self.plugin.config.dashboard_secret}"
