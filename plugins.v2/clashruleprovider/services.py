...
class ClashRuleProviderService:
    ...
    def parse_proxies_from_input(params: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:
        ...
        if params.get('type') == 'YAML':
            try:
                ...
                logger.error(f"Failed to load YAML payload: {repr(params)}: {repr(err)}")
                return None
    ...
    @staticmethod
    def notify_clash(ruleset: str, api_url: str, api_secret: str):
        """
        通知 Clash 刷新规则集
        """
    ...
    def add_notification_job(self, ruleset_names: List[str]):
        ...
        for ruleset in ruleset_names:
            if ruleset in self.plugin.state.rule_provider:
                self.plugin.scheduler.add_job(ClashRuleProviderService.notify_clash, "date",
                                              run_date=datetime.now(
                                                  tz=pytz.timezone(settings.TZ)) + timedelta(
                                                  seconds=self.plugin.config.refresh_delay),
    ...
    async def async_refresh_acl4ssr(self):
        ...
        new_providers = {}
        for path in paths:
            response = await AsyncRequestUtils().get_res(api_url % path, headers=settings.GITHUB_HEADERS, params={'ref': branch})
            if not response:
                continue
            files = response.json()
