...
class ClashRuleProvider(_ClashRuleProviderBase):
    ...
    def _start_scheduler(self):
        ...
        self.scheduler.add_job(self.refresh_subscriptions, "date", run_date=now + timedelta(seconds=2))
        if self.config.hint_geo_dat:
            self.scheduler.add_job(self.refresh_geo_dat, "date", run_date=now + timedelta(seconds=3))
        else:
            self.state.geo_rules = {'geoip': [], 'geosite': []}
        if self.config.enable_acl4ssr:
            self.scheduler.add_job(self.refresh_acl4ssr, "date", run_date=now + timedelta(seconds=4))
        else:
            self.state.acl4ssr_providers = {}

    def refresh_subscriptions(self):
        asyncio.run(self.services.async_refresh_subscriptions())

    def refresh_geo_dat(self):
        asyncio.run(self.services.async_refresh_geo_dat())

    def refresh_acl4ssr(self):
        asyncio.run(self.services.async_refresh_acl4ssr())
