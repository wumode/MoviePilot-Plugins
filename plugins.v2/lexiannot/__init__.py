...
class LexiAnnot(_PluginBase):
    ...
    def init_plugin(self, config=None):
        ...
        if config:
            ...
            self._context_window = int(config.get("context_window"))
            self._max_retries = int(config.get("max_retries"))
            self._request_interval = int(config.get("request_interval"))
    ...
    def __process_by_ai(self, lines_to_process: List[Dict[str, Any]],
        ...
        patterns = [r'\d+th|\d?1st|\d?2nd|\d?3rd', r"\w+'t$", "[Ii]'m$", r"\w+'re$", r"\w+'ve$", r"\w+'ll$"]
